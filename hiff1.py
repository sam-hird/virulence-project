...
def mainLoop(nIters, iterOffset, hostList, paraList, virulence):
  ...
  for iteration in range(iterOffset, nIters):
    ...
    for host in hostList:
      ...
      hostScores.append([iteration, host.score])
    for para in paraList:
      paraNum1s.append([iteration,para.bitList.count(1)])
      paraScores.append([iteration, para.score])    
...
hostScores              = [] #[[iteration, score]] 
paraScores              = [] #[[iteration, score]]   
...
f, (ax0, ax1, ax2, ax3, ax4) = plt.subplots(5, 1, sharex=True, subplot_kw={'xlim': {0,GENERATIONS}}, gridspec_kw={'height_ratios': [5, 5, 5, 5, 1]}, figsize=(5, 13))

#plot num1s
ax0.title.set_text("Proportion of 1s in genotype, virulence = " + str(VIRULENCE))
ax0.set_ylim([0,1])
...
#plot conectedness
ax3.title.set_text("population scores")
ax3.set_ylim([0,COMPETITION_SIZE])
ax3.plot([a[0] for a in hostScores], [a[1] for a in hostScores], 'o', color='red', markersize=0.1, clip_on=False, zorder=100);
ax3.plot([a[0] for a in paraScores], [a[1] for a in paraScores], 'o', color='blue', markersize=0.1, clip_on=False, zorder=100);
